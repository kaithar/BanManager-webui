...
if(!isset($_SESSION['admin']) || (isset($_SESSION['admin']) && !$_SESSION['admin']))
	...
	if(!connect($server))
		...
		$pastBans = mysql_query("SELECT ban_record_id FROM ".$server['ipRecordTable']." WHERE ban_record_id = '".$_GET['id']."'");
		
		if(mysql_num_rows($pastBans) == 0)
			$error = 'That record does not exist';
		else {
			mysql_query("DELETE FROM ".$server['ipRecordTable']." WHERE ban_record_id = '".$_GET['id']."'");
